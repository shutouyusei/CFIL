...
class NesEnv(gym.Env, utils.EzPickle):
    ...
    def _create_pipes(self):
        ...
        import uuid 
        self.pipe_name = uuid.uuid4().hex
        self.launch_vars['pipe_name'] = self.pipe_name
        if not self.disable_out_pipe:
            self.path_pipe_out = '%s-out.%s' % (self.path_pipe_prefix, self.pipe_name)
    ...
    def _listen_to_incoming_pipe(self, pipe_name):
        ...
        self.path_pipe_in = '%s-in.%s' % (self.path_pipe_prefix, self.pipe_name)
    ...
    def _launch_fceux(self):
        ...
        import uuid
        self.temp_lua_path = os.path.join('/tmp',f"{uuid.uuid4().hex}.lua")
    ...
    def step(self, action):
        ...
        if self.first_step:
            self.first_step = False
            self.curr_seed = self.np_random.integers(0,256)
    ...
    def _seed(self, seed=None):
        ...
        self.curr_seed = actual_seed % 256
...
class MetaNesEnv(NesEnv):
    ...
    def step(self, action):
        ...
        obs, step_reward, is_finished,truncated, info = NesEnv.step(self, action)
