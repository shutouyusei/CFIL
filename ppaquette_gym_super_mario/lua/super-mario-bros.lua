...
function get_tiles()
	...
	gui.box(50 - 5 * 7 - 2, 70 - 5 * 7 - 2, 50 + 5 * 8 + 3, 70 + 5 * 5 + 3, 0, "P30") -- P30 = White (NES Palette 30 color)
	...
	for box_y = -4 * 16, 8 * 16, 16 do
		...
		for box_x = -7 * 16, 8 * 16, 16 do
			...
			if (framecount % skip_frames == 0) and (skip_tiles == 0) then
				...
				if
					(framecount % send_all_pixels == 0)
					or (tile_value ~= tiles[(box_x / 16) + 7][(box_y / 16) + 4])
					or (force_refresh > 0)
				then
					...
					tile_string = tile_string
						.. "|"
						.. string.format("%01x%01x%01x", (box_x / 16) + 7, (box_y / 16) + 4, tile_value)
					data_count = data_count + 1
				end
			end
		end
...
function check_if_finished()
	if
		...
		or (get_life() < 3)
		or (get_level() > 4 * (target_world - 1) + (target_level - 1))
	then
		-- Level finished
